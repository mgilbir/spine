...
- **Word (DOCX)**: Create and modify Word documents
  ...
  - Document structure parts: read building blocks / AutoText from the glossary (`Document.BuildingBlocks()` — name, gallery, category, types, guid), read and add custom-XML data parts (`Document.CustomXMLParts()` / `Document.AddCustomXMLPart` — the latter generates the itemProps, relationships, and content-type override), and read the web-layout frameset tree (`Document.Frameset()` — nested framesets and leaf frames with their resolved source documents). Glossary, custom-XML, and frameset parts round-trip byte-for-byte; building blocks and framesets are read-only
