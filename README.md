...
- **Embedded OLE Objects**: read embedded OLE objects (`embeddings/oleObjectN.bin`) with `OLEObjects()`, returning each object's name, content type, raw bytes, and best-effort ProgID; unmodified objects round-trip verbatim (embedding new objects is not yet supported)
