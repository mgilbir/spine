...
// Real-world DOCX packages (Common Crawl corpus) carry junk zip entries like
...
func TestRoundTripPreservesPartWithoutContentType(t *testing.T) {
	fixture := testutil.AppendZipEntry(t, "testdata/minimal.docx", "[trash]/0000.dat", []byte{0xde, 0xad, 0xbe, 0xef})

	d, err := OpenReader(bytes.NewReader(fixture), int64(len(fixture)))
	if err != nil {
