...
func (img *InlineImage) applyEdit(patch func([]byte) []byte) {
	...
	// Flag here rather than in each caller: an image edit through a handle into
	// a reopened header/footer must regenerate that part, and the next editing
	// accessor added would otherwise have to remember on its own (C406).
	img.touch()
	if img.parsed {
...
func (r *Run) addImageData(data []byte, contentType, ext string, anchor Anchor, floating bool) (*InlineImage, error) {
	...
	img.drawing = drawing
	// mut(), not r.r: a drawing added to a reopened header/footer run must
	// regenerate that part on save; otherwise the media part and its
	// relationship are written but the preserved header bytes (lacking the
	// drawing) win, orphaning both.
	r.mut().AppendDrawing(drawing)
	return img, nil
}
...
func (r *Run) addSVGImageData(svgData, fallbackData []byte, fallbackCT string, anchor Anchor, floating bool) (*InlineImage, error) {
	...
	drawing := &oxml.CT_Drawing{RawContent: img.buildDrawingXML()}
	img.drawing = drawing
	// See addImageData: mut() flags a reopened header/footer part so its
	// regenerated bytes carry the new drawing (and its image relationships
	// resolve).
	r.mut().AppendDrawing(drawing)
	return img, nil
}
