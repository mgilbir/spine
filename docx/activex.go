...
// ActiveXControl is an ActiveX control embedded in a document: the ax:ocx
// control part (word/activeX/activeXN.xml) plus its persistence binary
...
// ActiveXControls returns the document's ActiveX controls, ordered by part name
// for determinism. Controls are located by their ax:ocx content type (or the
...
func (d *Document) ActiveXControls() []ActiveXControl {
	...
	for _, name := range names {
		...
		if binName, binData := d.resolveActiveXBinary(name); binName != "" {
...
func (d *Document) resolveActiveXBinary(xmlPart string) (string, []byte) {
	for _, rel := range d.relationships[xmlPart] {
		if rel == nil || rel.TargetMode == opc.TargetModeExternal {
			continue
		}
		target := opc.ResolvePartName(xmlPart, rel.Target)
		if part, ok := d.preservedParts[target]; ok && (strings.HasSuffix(strings.ToLower(target), ".bin") || part.ContentType == contentTypeActiveXBin) {
			return target, part.Data
		}
	}
	if i := strings.LastIndex(strings.ToLower(xmlPart), ".xml"); i >= 0 {
		sibling := xmlPart[:i] + ".bin"
		if part, ok := d.preservedParts[sibling]; ok {
