...
type ClientData struct {
	...
	// MoveWithCells/SizeWithCells/AutoFill/MultiLine are usually empty presence
	// flags (<x:MoveWithCells/>). Modeled as string,omitempty they unmarshaled
	// to "" and were then dropped on marshal, losing the flag's presence. As
	// *string a present-but-empty element is a non-nil pointer to "" (emitted),
	// an absent one is nil (omitted), and a valued form (e.g. AutoFill=False)
