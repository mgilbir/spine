...
func TestModernComment_RoundTrip(t *testing.T) {
	tests := []struct {
		name string
		xml  string
	}{
		{
			name: "basic modern comment",
			xml:  `<comment id="comment1" authorId="author1" status="active" created="2024-01-15T10:30:00Z"><text>Modern comment</text></comment>`,
		},
		{
			name: "reply comment",
			xml:  `<comment id="comment2" authorId="author1" parentId="comment1"><text>Reply</text></comment>`,
		},
		{
			name: "resolved comment",
			xml:  `<comment id="comment3" authorId="author2" status="resolved" done="true"><text>Done</text></comment>`,
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			var mc ModernComment
			if err := xml.Unmarshal([]byte(tt.xml), &mc); err != nil {
				t.Fatalf("Unmarshal failed: %v", err)
			}
			...
			var mc2 ModernComment
			if err := xml.Unmarshal(out, &mc2); err != nil {
				t.Fatalf("Re-unmarshal failed: %v", err)
			}
		})
	}
...
func TestModernCommentAuthor_RoundTrip(t *testing.T) {
	tests := []struct {
		id         string
		name       string
		userId     string
		...
		{"author1", "John Doe", "user123", "provider1"},
		{"author2", "Jane Smith", "", ""},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			mca := &ModernCommentAuthor{
				Id:         tt.id,
				Name:       tt.name,
				UserId:     tt.userId,
				ProviderId: tt.providerId,
			}
			out, err := xml.Marshal(mca)
			if err != nil {
				t.Fatalf("Marshal failed: %v", err)
			}

			var mca2 ModernCommentAuthor
			if err := xml.Unmarshal(out, &mca2); err != nil {
				t.Fatalf("Unmarshal failed: %v", err)
			}

			if mca2.Id != tt.id {
				t.Errorf("Id = %q, want %q", mca2.Id, tt.id)
			}
			if mca2.Name != tt.name {
				t.Errorf("Name = %q, want %q", mca2.Name, tt.name)
			}
		})
	}
}
