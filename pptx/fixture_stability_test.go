...
func TestPptxFuzzFixturesAreByteStableAcrossASecond(t *testing.T) {
	if testing.Short() {
		t.Skip("sleeps for a second to cross a wall-clock boundary")
	}
	build := func() map[string][]byte {
		return map[string][]byte{
			"buildValidPptxFuzzSeed": buildValidPptxFuzzSeed(t),
			"buildFuzzDeck":          buildFuzzDeck(t),
		}
	}
	before := build()
	...
	for name, first := range before {
		...
		for _, part := range []string{
			"docProps/core.xml", "docProps/app.xml", "ppt/presentation.xml",
