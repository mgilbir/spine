...
func (s *Slide) applyAnimations() {
	...
	for _, a := range anims {
		...
		// Guard against an unresolved target: a shape id of 0 is never a valid
		// cNvPr id (the shape tree reserves 1, shapes start at 2), so it means the
...
// with the given cNvPr id, or 0 when the shape has no text body. It descends
// into groups so a build-by-paragraph target nested in a p:grpSp is found (C354).
func (s *Slide) paragraphCount(spid uint32) int {
	if s.sx() == nil || s.sx().CSld == nil || s.sx().CSld.SpTree == nil {
		return 0
	}
	tree := s.sx().CSld.SpTree
	if n, ok := paragraphCountInShapes(tree.Sp, spid); ok {
		return n
	}
	for _, grp := range tree.GrpSp {
		if n, ok := paragraphCountInGroup(grp, spid); ok {
			return n
		}
	}
	return 0
...
// paragraphCountInShapes reports the paragraph count of the shape matching spid
// among the given shapes, and whether a match was found.
func paragraphCountInShapes(shapes []*oxml.Shape, spid uint32) (int, bool) {
	for _, sp := range shapes {
		if sp.NvSpPr != nil && sp.NvSpPr.CNvPr != nil && sp.NvSpPr.CNvPr.Id == spid {
			if sp.TxBody != nil {
				return len(sp.TxBody.P), true
			}
			return 0, true
		}
	}
...
// paragraphCountInGroup searches a group (and its nested groups) for the shape
// matching spid, reporting its paragraph count and whether it was found.
func paragraphCountInGroup(grp *oxml.GroupShape, spid uint32) (int, bool) {
	if grp == nil {
		return 0, false
	}
	if n, ok := paragraphCountInShapes(grp.Shapes, spid); ok {
		return n, true
	}
	for _, sub := range grp.GroupShapes {
		if n, ok := paragraphCountInGroup(sub, spid); ok {
			return n, true
		}
	}
	return 0, false
