...
// preferSource returns the part's original bytes when the regenerated form says
// the same thing, and the regenerated bytes otherwise.
//
// presentation.xml, slide layouts and slide masters are rebuilt from the model
// on every save — unlike slides, which pass through verbatim while untouched —
// because the writer owns their id lists and relationship ids. The rebuild
// emits one canonical serialization, so a producer that pretty-printed those
// parts saw its indentation deleted by a zero-modification round trip (C587),
// and one that mixed <a:pPr/> with <a:pPr></a:pPr>, or wrote a bullet as
// &#x2022;, saw those normalized too.
//
// The guard is the strictest one that fixes it: byte-for-byte equality of every
// token, forgiving only the spellings XML defines as interchangeable (see
// xmlb.EqualIgnoringIndent). Any real change to the model — an edited
// attribute, an added shape, a dropped child — makes the two forms differ by
// more than that, and the regenerated bytes win as before. A deck built
// programmatically has no source, so it always emits canonically.
func preferSource(source, regenerated []byte) []byte {
	if len(source) > 0 && xmlb.EqualIgnoringIndent(source, regenerated) {
		return source
	}
	return regenerated
}
...
func marshalSlideLayout(layout *oxml.SlideLayout) ([]byte, error) {
	...
	return preferSource(layout.SourceXML, b.Bytes()), nil
}
...
func marshalSlideMaster(master *oxml.SlideMaster) ([]byte, error) {
	...
	return preferSource(master.SourceXML, b.Bytes()), nil
}
...
func marshalPresentationXML(pres *oxml.Presentation, synthesizeDefaults bool) ([]byte, error) {
	...
	return preferSource(pres.SourceXML, b.Bytes()), nil
}
