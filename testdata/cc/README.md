...
This README covers fetching the corpus and running the tests — everything a
contributor needs. Maintainer harvest and regeneration operations (the
batched multi-crawl sweep, the stress set, and the resource-capped `ccrun`
batch runner) live in a separate guide: [**HARVEST.md**](HARVEST.md).
...
yield larger documents. `make fetch-cc` can optionally refetch the rows in the
`manifest-<type>-truncated.tsv` manifests from their *original URL* on the live
web — behind a mandatory DNS-over-HTTPS blocklist gate, with reduced
concurrency, a hard size cap, and its own `-live-n` limit. This live phase
refuses to run without a resolver URL and is inherently less reproducible than
WARC fetching. See [HARVEST.md](HARVEST.md#live-fetch-of-truncated-candidates)
for the DoH gate, politeness, and cap details.
...
## Harvest and regeneration operations

Everything above is what a contributor needs. The maintainer-only operations —
live-fetching truncated candidates in full, regenerating the manifests with the
batched multi-crawl sweep, the docx/xlsx stress set, and the resource-capped
`ccrun` fetch→test→record→discard harness — are documented in
